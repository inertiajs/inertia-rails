...
function extractDocs() {
  ...
  output += 'Note: Code blocks are preserved and formatted for LLM consumption.\n\n';
...
function generateCrossReferenceIndex() {
  ...
  for (const filePath of walkDir(DOCS_DIR)) {
    ...
    if (markdownContent.trim()) {
      ...
      const codeBlocks = cleanContent.match(/CODE BLOCK \([^)]+\):\n([\s\S]*?)(?=\n\n|\n#|$)/g) || [];
      const methodNames = [];
      const componentNames = [];
      
      codeBlocks.forEach(block => {
        // Extract Rails method names
        const railsMethods = block.match(/def \w+|render inertia:|inertia_share|router\.\w+/g) || [];
        methodNames.push(...railsMethods);
        
        // Extract component names
        const components = block.match(/[A-Z][a-zA-Z]*(?:Component|Page|Layout)/g) || [];
  ...
  const functionalGroups = {
    'Authentication & Authorization': ['authentication', 'authorization', 'csrf', 'current_user', 'authenticate'],
    'Forms & Validation': ['forms', 'validation', 'useForm', 'errors', 'redirect_back'],
    'Routing & Navigation': ['routing', 'links', 'router.visit', 'manual-visits', 'redirects'],
    'Data Management': ['responses', 'props', 'shared-data', 'partial-reloads', 'deferred-props'],
...
function generateAPIReference() {
  ...
  for (const filePath of walkDir(DOCS_DIR)) {
    ...
    const rubyBlocks = cleanContent.match(/CODE BLOCK \(ruby\):\n([\s\S]*?)(?=\n\n|\n#|$)/g) || [];
    rubyBlocks.forEach(block => {
      ...
      const configBlocks = code.match(/InertiaRails\.configure[\s\S]*?end/g) || [];
    ...
    const jsBlocks = cleanContent.match(/CODE BLOCK \((js|javascript|typescript|jsx|tsx)\):\n([\s\S]*?)(?=\n\n|\n#|$)/g) || [];
